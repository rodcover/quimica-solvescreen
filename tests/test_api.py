...
def client(tmp_path, monkeypatch):
    ...
    model = r.pop("model")
    p = tmp_path / "m.joblib"
    joblib.dump({"model": model, "feature": "test"}, p)
...
def test_predict_bulk(client: TestClient):
    r = client.post("/predict/bulk", json={"solute_smiles": "CCO"})
    assert r.status_code == 200
    data = r.json()
    assert "delta_g_kcal_mol" in data
